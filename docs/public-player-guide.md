...
## Plans, gear, and requests

The Calculator supports single and multi-item plans. Gear can be used to build a plan, Requests can track local production work, and the reference tabs provide item, faction, mining, and knowledge-base context. These workflows remain usable without login or a shared backend.

## Workspace portability

Use `Export workspace` for a complete local snapshot containing supported player profiles, inventory, plans, requests, gear, preferences, analytics settings, and colony-world context. Use `Import workspace` on another browser to restore it. Imports are validated before storage mutation. Legacy inventory-only JSON remains supported.
